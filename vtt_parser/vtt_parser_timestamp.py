...
def extractLine(srcPath):
    ...
    for caption in vtt:
        for line in caption.text.strip().splitlines():
            # print(caption.start)
            yield caption.start.split(".")[0] + ": " + line
...
def filterLines(lines):
    ...
    for line in lines:
        if line.split(": ")[1] == lastLine or line.split(": ")[1] in filteredTerms:
            # print(line)
            continue
...
for dir1 in os.listdir(srcPath):
    for file in os.listdir(srcPath + "/" + dir1):
        if file.endswith(".en.vtt") and os.path.exists(srcPath + "/" + dir1 + "/" + "category2"):
            # print(file.split(" ")[0])
            parse_vtt(srcPath + "/" + dir1 + "/" + file, Path(destPath + "/" + file.split(" ")[0] + "_timestamp.txt"))